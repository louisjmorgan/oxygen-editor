...
async function saveNewTree(name: string, tree: Node) {
  ...
  if (saveResponse.success === false) throw new Error(`Error saving tree: ${saveResponse.msg}`)

  const fetchResponse = await fetchTrees()
  if (fetchResponse.success === false) throw new Error(`Error saving tree: ${fetchResponse.msg}`)
...
async function deleteTree(loaded: LoadedTree) {
  ...
  if (deleteResponse.success === false) throw new Error(`Error saving tree: ${deleteResponse.msg}`)
