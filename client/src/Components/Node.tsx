...
import {ACTIONTYPE, AddressMap, AddressMapItem, Node as NodeType} from "../Model/Types"
...
const Node = ({
  ...
  const focusSelf = (event: React.MouseEvent<HTMLButtonElement | HTMLDivElement>) => {
  ...
  const childNodes = [...node.children].map(([name, child]: [string, NodeType]) => (
    <Node
      node={child}
      dispatch={dispatch}
      addressMap={addressMap}
      visible={displayChildren}
      displayChildren={addressMap.get(child.address.toString())?.display as boolean}
      key={child.name}
      ...
      <div 
        ...
        >
          ...
          <NameField 
            ...
            dispatch={dispatch}
            showTools={tools}
            edit={addressMap.get(node.address.toString())?.editName as boolean}
        ...
        <InsertNodeField 
            node={node}
            dispatch={dispatch}
            addressMap={addressMap.get(node.address.toString()) as AddressMapItem}
            // active={addressMap.get(node.address.toString())?.editNode as boolean}
            // target={addressMap.get(node.address.toString())?.insertTarget as "sibling" | "child"}
            // input={addressMap.get(node.address.toString())?.inputNode as string}
            submitInsertChild={submitInsertChild}
